...
export async function GET(req) {
    ...
    `);
        ...
        const pieWidgets = widgets.filter((w) => w?.kind === 'api_pie');
        // Best-effort: show last applied Drizzle migrations (so we can confirm 0015 ran)
        let drizzleMigrations = null;
        const candidates = ['__drizzle_migrations', 'drizzle.__drizzle_migrations'];
        for (const table of candidates) {
            try {
                const r = await pool.query(`SELECT * FROM ${table} ORDER BY created_at DESC LIMIT 10;`);
        ...
        return NextResponse.json({
            ...
            pie_charts: pieWidgets.map((w) => ({
                key: w?.key,
                title: w?.title,
                endpoint: w?.presentation?.endpoint,
            })),
            all_widget_kinds: widgets.map((w) => w?.kind).filter(Boolean),
